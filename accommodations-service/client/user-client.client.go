...
func (uc UserClient) GetUserById(ctx context.Context, userId string) (*HostUser, *errors.ErrorStruct) {
	cbResp, err := uc.circuitBreaker.Execute(func() (interface{}, error) {
		req, err := http.NewRequestWithContext(ctx, http.MethodGet, uc.address+"/"+userId, nil)
		if err != nil {
			log.Println(err)
			return nil, err
		}
